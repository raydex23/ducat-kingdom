...
export default function PleasureHouse() {
  ...
  const rooms = [
    {
      ...
      bonusEnergy: 20,
      bonusPower: 10,
      flavor: "Cheapest, but after few beers in Tavern it doesn't make a difference.",
    },
    {
      id: 2,
      name: "Rowena",
      ...
      bonusEnergy: 40,
      bonusPower: 25,
      flavor: "Very busy, miners favourite!",
    },
    {
      id: 3,
      name: "Alinor",
      ...
      bonusEnergy: 70,
      bonusPower: 50,
      flavor: "Your chubby dream. Let her take care of you.",
    },
    {
      id: 4,
      name: "Seraphine",
      ...
      bonusEnergy: 100,
      bonusPower: 90,
      flavor: "Visited only by nobles, her aura alone inspires greatness.",
    },
    {
      id: 5,
      name: "Isolde",
  ...
  const randomEvent = [
    { msg: "💃 A mysterious guest tips generously (+5 $CROWN)", effect: (s) => (s.balance += 5) },
    { msg: "🍷 Your girls overhear trade secrets — upgrades 10% cheaper today!", effect: (s) => (s.discount = Date.now()) },
  ...
  <div className="min-h-screen bg-gradient-to-b from-[#1b1815] via-[#2a2520] to-[#14110f] text-gray-200 p-6">
    ...
    <div className="p-6 bg-[#2a2520]/80 border border-[#3b332b] rounded-2xl shadow-lg max-w-3xl mx-auto text-center">
      {/* Booster pack image */}
      <img
        src="/images/hookers/booster_pack.png"
        alt="Pleasure Pack"
      ...
      <p className="text-sm text-gray-400 mb-4">
        Open a Pleasure Pack to reveal a harlot and permanent bonuses to your miners.
      </p>
      ...
      {result && (
        <div className="mt-6 p-5 border border-[#5a4a3b]/70 rounded-xl bg-[#1f1a16]/70 shadow-inner animate-fadeIn">
          {/* Room image */}
          <img
            src={`/images/hookers/hooker_${result.id}.png`}
            alt={result.name}
            className="w-full h-40 object-cover rounded-lg border border-[#3b332b] shadow mb-3"
          />
          ...
          {result.duplicate ? (
            <p className="text-amber-300 font-mono">
              Duplicate! You received 1000 $CROWN compensation.
            </p>
          ) : (
            <p className="text-amber-300 font-mono">
    ...
    {state.pleasureRooms?.length > 0 && (
      <div className="mt-8 max-w-3xl mx-auto bg-[#2a2520]/80 border border-[#3b332b] rounded-2xl shadow-lg p-5">
        <h3 className="text-lg font-semibold text-pink-300 mb-3">Owned Harlots</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {rooms
            ...
            .map((r) => (
              <div
                key={r.id}
                className="p-3 bg-[#1f1a16]/60 border border-[#3b332b] rounded-xl text-left flex gap-3 items-center"
              >
                <img
                  src={`/images/hookers/hooker_${r.id}.png`}
                  alt={r.name}
